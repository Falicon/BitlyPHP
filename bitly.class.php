...
class PHPBitlyAPI
{
    ...
    public $API = array("KEY" => "R_685583af62c827761fd45d693fb3d617", "LOGIN" => "falicon", "CLIENT" => array("ID" => "2f4b1eb5beb9b615ac70b16e34daf4bc6b899112", "SECRET" => "149e58cfb3f5d508386822be40afc8ce75ca1c26"));
    public $API_ADDRESS = array("MAIN" => array("API" => "http://api.bit.ly/v3/"), "oAUTH" => array("API" => "https://api-ssl.bit.ly/v3/", "ACCESS_TOKEN" => "https://api-ssl.bit.ly/oauth/"));
     ...
     * Example usage:
     * @code 
     *  $bitlyphp->setAPI("KEY", "R_685583af62c827761fd45d693fb3d617");
     *  $bitlyphp->setAPI("LOGIN", "falicon");
     *  $bitlyphp->setAPI("CLIENT->ID", "example");
     *  $bitlyphp->setAPI("CLIENT->SECRET", "example");
     *
     * @return
     *   False:
     *   - If any criteria can't be matched. Criteria is first parameter which decides which property will be updated.
     *
     */
    public function setAPI($x, $val)
    {
        switch ($x) {
            ...
            case "CLIENT->ID":
                $this->API['CLIENT']['ID'] = $val;
                break;
            
            case "CLIENT->SECRET":
                $this->API['CLIENT']['SECRET'] = $val;
                break;
    ...
    /**
     * Setup API ADDRESSEs (calls)
     *
     * Example usage:
     * @code 
     *  $bitlyphp->setAPI("MAIN->API", "https://example");
     *  $bitlyphp->setAPI("oAUTH->API", "https://example");
     *  $bitlyphp->setAPI("oAUTH->ACCESS_TOKEN", "https://example");     
     *
     * @return
     *   False:
     *   - If any criteria can't be matched. Criteria is first parameter which decides which property will be updated.
     *
     */
    public function setAPI_ADDRESS($x, $val)
    {
        switch ($x) {
            
            case "MAIN->API":
                $this->API_ADDRESS['MAIN']['API'] = $val;
                break;
            
            case "oAUTH->API":
                $this->API_ADDRESS['oAUTH']['API'] = $val;
                break;
            
            case "oAUTH->ACCESS_TOKEN":
                $this->API['oAUTH']['ACCESS_TOKEN'] = $val;
                break;
     ...
     */
    function shorten($longUrl, $access_token, $domain = '', $x_login = '', $x_apiKey = '')
    {
        $result = array();
        $url    = $this->API_ADDRESS['oAUTH']['API'] . "shorten?access_token={$access_token}&longUrl=" . urlencode($longUrl);
        if ($domain != '') {
            $url .= "&domain={$domain}";
        }
        if ($x_login != '' && $x_apiKey != '') {
    ...
    function expand($data)
    {
        ...
        $url    = $this->API_ADDRESS['MAIN']['API'] . "expand?login={$this->API['LOGIN']}&apiKey={$this->API['KEY']}&format=json&hash={$data}";
        $output = json_decode($this->get_curl($url));
    ...
    {
        $result = 0;
        $url    = $this->API_ADDRESS['MAIN']['API'] . "validate?login={$this->API['LOGIN']}&apiKey={$this->API['KEY']}&format=json&x_login={$x_login}&x_apiKey={$x_apiKey}";
        $output = json_decode($this->get_curl($url));
    ...
    function clicks($data)
    {
        ...
        $url    = $this->API_ADDRESS['MAIN']['API'] . "clicks?login={$this->API['LOGIN']}&apiKey={$this->API['KEY']}&format=json&hash={$data}";
        $output = json_decode($this->get_curl($url));
    ...
    function referrers($data)
    {
        ...
        $tmp     = array_reverse($tmp);
        $data    = $tmp[0];
        $url     = $this->API_ADDRESS['MAIN']['API'] . "referrers?login={$this->API['LOGIN']}&apiKey={$this->API['KEY']}&format=json&hash={$data}";
        $output  = json_decode($this->get_curl($url));
    ...
    function countries($data)
    {
        ...
        $tmp     = array_reverse($tmp);
        $data    = $tmp[0];
        $url     = $this->API_ADDRESS['MAIN']['API'] . "countries?login={$this->API['LOGIN']}&apiKey={$this->API['KEY']}&format=json&hash={$data}";
        $output  = json_decode($this->get_curl($url));
    ...
    function clicks_by_minute($data)
    {
        ...
        $url    = $this->API_ADDRESS['MAIN']['API'] . "clicks_by_minute?login={$this->API['LOGIN']}&apiKey={$this->API['KEY']}&format=json&hash={$data}";
        $output = json_decode($this->get_curl($url));
    ...
    function clicks_by_day($data, $days = 7)
    {
        ...
        $url    = $this->API_ADDRESS['MAIN']['API'] . "clicks_by_day?login={$this->API['LOGIN']}&apiKey={$this->API['KEY']}&format=json&days={$days}&hash={$data}";
        $output = json_decode($this->get_curl($url));
    ...
    {
        $result = array();
        $url    = $this->API_ADDRESS['MAIN']['API'] . "bitly_pro_domain?login={$this->API['LOGIN']}&apiKey={$this->API['KEY']}&format=json&domain={$domain}";
        $output = json_decode($this->get_curl($url));
    ...
    function lookup($data)
    {
        ...
        $url    = $this->API_ADDRESS['oAUTH']['API'] . "lookup?url={$data}&access_token={$this->API['KEY']}";
        $output = json_decode($this->get_curl($url));
    ...
    {
        $result               = array();
        $url                  = $this->API_ADDRESS['MAIN']['API'] . "authenticate";
        $params               = array();
        $params['login']      = $this->API['LOGIN'];
        $params['apiKey']     = $this->API['KEY'];
        $params['format']     = "json";
        $params['x_login']    = $x_login;
        $params['x_password'] = $x_password;
        $output               = json_decode($this->post_curl($url, $params));
    ...
    function info($data)
    {
        ...
        // make the call to expand
        $url    = $this->API_ADDRESS['MAIN']['API'] . "info?login={$this->API['LOGIN']}&apiKey={$this->API['KEY']}&format=json&hash={$data}";
        $output = json_decode($this->get_curl($url));
    ...
    {
        $results                 = array();
        $url                     = $this->API_ADDRESS['oAUTH']['ACCESS_TOKEN'] . "access_token";
        $params                  = array();
        $params['client_id']     = $this->API['CLIENT']['ID'];
        $params['client_secret'] = $this->API['CLIENT']['SECRET'];
        $params['code']          = $code;
        $params['redirect_uri']  = $redirect;
        $output                  = $this->post_curl($url, $params);
     ...
     */
    function user_clicks($access_token, $days = 7)
    {
        // $results = user_clicks('BITLY_SUPPLIED_ACCESS_TOKEN');
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "user/clicks?access_token={$access_token}&days={$days}";
        $output  = json_decode($this->get_curl($url));
        if (isset($output->{'data'}->{'clicks'})) {
     ...
     */
    function user_referrers($access_token, $days = 7)
    {
        // $results = user_referrers('BITLY_SUPPLIED_ACCESS_TOKEN');
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "user/referrers?access_token={$access_token}&days={$days}";
        $output  = json_decode($this->get_curl($url));
        if (isset($output->{'data'}->{'referrers'})) {
            $results['days']      = $output->{'data'}->{'days'};
            $results['referrers'] = array();
     ...
     */
    function user_countries($access_token, $days = 7)
    {
        // $results = user_countries('BITLY_SUPPLIED_ACCESS_TOKEN');
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "user/countries?access_token={$access_token}&days={$days}";
        $output  = json_decode($this->get_curl($url));
        if (isset($output->{'data'}->{'countries'})) {
            $results['days']      = $output->{'data'}->{'days'};
            $results['countries'] = array();
     ...
     */
    function user_realtime_links($access_token)
    {
        // $results = user_realtime_links('BITLY_SUPPLIED_ACCESS_TOKEN');
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "user/realtime_links?format=json&access_token={$access_token}";
        $output  = json_decode($this->get_curl($url));
     ...
     */
    function user_link_history($access_token)
    {
        // $results = user_link_history('BITLY_SUPPLIED_ACCESS_TOKEN');
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "user/link_history?format=json&access_token={$access_token}";
        $output  = json_decode($this->get_curl($url));
     ...
     */
    function highvalue($access_token, $limit = 5)
    {
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "highvalue?access_token={$access_token}&limit={$limit}";
        $output  = json_decode($this->get_curl($url));
     ...
     * @see http://dev.bitly.com/data_apis.html
     */
    function search($access_token, $query, $limit = 10, $offset = 0, $domain = '', $lang = '', $cities = '', $fields = array())
    {
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "search?access_token={$access_token}&query=" . urlencode($query) . "&limit={$limit}&offset={$offset}";
        if ($domain != '') {
     ...
     * @see http://dev.bitly.com/data_apis.html
     */
    function realtime_bursting_phrases($access_token)
    {
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "realtime/bursting_phrases?access_token={$access_token}";
        $output  = json_decode($this->get_curl($url));
     ...
     * @see http://dev.bitly.com/data_apis.html
     */
    function realtime_hot_phrases($access_token)
    {
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "realtime/bursting_phrases?access_token={$access_token}";
        $output  = json_decode($this->get_curl($url));
     ...
     * @see http://dev.bitly.com/data_apis.html
     */
    function realtime_clickrate($access_token, $phrase)
    {
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "realtime/clickrate?access_token={$access_token}&phrase=" . urlencode($phrase);
        $output  = json_decode($this->get_curl($url));
     ...
     * @see http://dev.bitly.com/data_apis.html
     */
    function link_info($access_token, $link)
    {
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "link/info?access_token={$access_token}&link=" . urlencode($link);
        $output  = json_decode($this->get_curl($url));
     ...
     * @see http://dev.bitly.com/data_apis.html
     */
    function link_content($access_token, $link)
    {
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "link/content?access_token={$access_token}&link=" . urlencode($link);
        $output  = json_decode($this->get_curl($url));
     ...
     * @see http://dev.bitly.com/data_apis.html
     */
    function link_category($access_token, $link)
    {
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "link/category?access_token={$access_token}&link=" . urlencode($link);
        $output  = json_decode($this->get_curl($url));
     ...
     * @see http://dev.bitly.com/data_apis.html
     */
    function link_social($access_token, $link)
    {
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "link/social?access_token={$access_token}&link=" . urlencode($link);
        $output  = json_decode($this->get_curl($url));
     ...
     * @see http://dev.bitly.com/data_apis.html
     */
    function link_location($access_token, $link)
    {
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "link/location?access_token={$access_token}&link=" . urlencode($link);
        $output  = json_decode($this->get_curl($url));
     ...
     * @see http://dev.bitly.com/data_apis.html
     */
    function link_language($access_token, $link)
    {
        $results = array();
        $url     = $this->API_ADDRESS['oAUTH']['API'] . "link/language?access_token={$access_token}&link=" . urlencode($link);
        $output  = json_decode($this->get_curl($url));
     ...
     */
    private function post_curl($uri, $fields)
    {
        $output        = "";
        $fields_string = "";
...
if (defined("bitlyPHP_init")) {
    ${bitlyPHP_init} = new PHPBitlyAPI; // Create instance
}